...
st.set_page_config(page_title="Маршрутна табличка (Альбомна)", layout="centered")
...
def create_landscape_page(v_main, v_left, v_right):
    ...
    try:
        ...
        st.error(f"Файл {font_file} не знайдено в репозиторії!")
        return None

    # --- 1. ВЕРХНІЙ БЛОК (Номер поїзда) ---
    bw, bh = mm(60), mm(25) # Трохи зменшив висоту для альбому
    ...
    # --- 3. ЦЕНТРАЛЬНЕ ЧИСЛО ---
    # У альбомному форматі менше місця по вертикалі, тому відступ 15мм
    ...
    if v_left:
        ...
        draw_arrow(draw, l_bbox[2] + mm(8), cy_sides, "left")
    ...
    if v_right:
        ...
        draw_arrow(draw, r_bbox[0] - mm(8), cy_sides, "right")

    return img

# Кнопка генерації
if st.button("🚀 Згенерувати А4 (Альбомна)"):
    zip_buf = io.BytesIO()
    with zipfile.ZipFile(zip_buf, "a", zipfile.ZIP_DEFLATED) as zip_file:
        for v in range(int(start_v), int(end_v) + 1):
            ...
            if v == start_v:
                st.image(img_a, caption=f"Попередній перегляд (Альбомна)")

    st.download_button("📥 Скачати ZIP", zip_buf.getvalue(), "labels_landscape.zip")
